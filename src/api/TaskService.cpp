...
TaskID TaskService::addTask(const TaskDTO &user_data) {
    ...
    return created_node.lock()->getId();
}

TaskID TaskService::addSubTask(TaskID id_parent, const TaskDTO &user_data) {
    Task ptask = task_creator_->createTask(user_data);
